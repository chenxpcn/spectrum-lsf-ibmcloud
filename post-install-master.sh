...
LOG "wget -nH -c --no-check-certificate -o $LOG_FILE -O /root/installer/${installer_name} ${installer_uri}"
wget -nv -nH -c --no-check-certificate -o $LOG_FILE -O "/root/installer/${installer_name}" ${installer_uri}
echo "1" > /root/installer/select_yes
chmod 744 "/root/installer/${installer_name}"
"/root/installer/${installer_name}" < /root/installer/select_yes
