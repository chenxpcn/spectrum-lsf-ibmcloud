...
LOG "Deploy provioning.sh for resource connector"
LOG "wget -nH -c --no-check-certificate -o $LOG_FILE -O /var/www/html/provisioning.sh $REMOTE_SCRIPT_PATH/provisioning.sh"
wget -nv -nH -c --no-check-certificate -o $LOG_FILE -O /var/www/html/provisioning.sh $REMOTE_SCRIPT_PATH/provisioning.sh
...
sed -i '/QUEUE_NAME \+= normal/a\RC_HOSTS        = softlayerhost; /RC_HOSTS        = softlayerhost/a\RC_ACCOUNT  = lsf-demo-dynamic-host' $LSF_ENVDIR/lsbatch/$CLUSTER_NAME/configdir/lsb.queues
...
badmin hclose lsf-master.hpc.ibmcloud>>$LOG_FILE

LOG "Restart the LSF daemons"
lsadmin limrestart>>$LOG_FILE
lsadmin resrestart>>$LOG_FILE
badmin mbdrestart>>$LOG_FILE
